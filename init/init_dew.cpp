...
void vendor_load_properties() {
    // Read the vendor name which distinguishes the rebadged brand tags
    const std::string vendor_name = GetProperty("ro.product.vendor.name", "");

    // Checking if the internal system branding reports as a poco configuration
    if (vendor_name.find("poco") != std::string::npos || vendor_name.find("c85") != std::string::npos) { 
